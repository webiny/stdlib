...
describe("HashFolderTool", () => {
    ...
    describe("hash (sync)", () => {
        it("returns a hex string for a folder with files", () => {
        ...
        it("returns a deterministic hash for the same content", () => {
            ...
            const hash1 = tool.hash(tmpDir);
            const hash2 = tool.hash(tmpDir);
        ...
        it("includes files in nested subdirectories", () => {
            ...
            const hashWithNested = tool.hash(tmpDir);

            writeFileSync(join(tmpDir, "sub", "nested.txt"), "changed");
            const hashAfterChange = tool.hash(tmpDir);
        ...
        it("excludes specified folders", () => {
            ...
            const hashWithExclude = tool.hash(tmpDir, { excludeFolders: ["dist"] });

            writeFileSync(join(tmpDir, "dist", "bundle.js"), "recompiled");
            const hashAfterDistChange = tool.hash(tmpDir, { excludeFolders: ["dist"] });
        ...
        it("is order-independent — same files in different creation order produce same hash", () => {
            ...
            const hash1 = tool.hash(dir1);
            const hash2 = tool.hash(dir2);
            expect(hash1).toBe(hash2);
        });
    ...
    describe("hashAsync (parallel)", () => {
        ...
        it("returns a deterministic hash for the same content", async () => {
            ...
            const hash1 = await tool.hashAsync(tmpDir);
            const hash2 = await tool.hashAsync(tmpDir);
            expect(hash1).toBe(hash2);
        });
        ...
        it("produces the same hash as the sync method", async () => {
            writeFileSync(join(tmpDir, "a.txt"), "alpha");
            mkdirSync(join(tmpDir, "sub"), { recursive: true });
            writeFileSync(join(tmpDir, "sub", "b.txt"), "beta");

            const syncHash = tool.hash(tmpDir);
            const asyncHash = await tool.hashAsync(tmpDir);
...
describe("hashFolder (sync standalone)", () => {
    ...
    it("returns a hash string directly", () => {
        writeFileSync(join(tmpDir, "file.txt"), "content");
    ...
    it("supports exclude options", () => {
        ...
        const hash1 = hashFolder(tmpDir, { excludeFolders: ["dist"] });

        writeFileSync(join(tmpDir, "dist", "out.js"), "recompiled");
        const hash2 = hashFolder(tmpDir, { excludeFolders: ["dist"] });

        expect(hash1).toBe(hash2);
    });

    it("produces the same hash as the DI tool", () => {
        writeFileSync(join(tmpDir, "file.txt"), "content");

        const tool = createHashFolderTool();
        const diHash = tool.hash(tmpDir);
        const standaloneHash = hashFolder(tmpDir);
...
describe("hashFolderAsync (async standalone)", () => {
    ...
    it("supports exclude options", async () => {
        ...
        const hash1 = await hashFolderAsync(tmpDir, { excludeFolders: ["dist"] });

        writeFileSync(join(tmpDir, "dist", "out.js"), "recompiled");
        const hash2 = await hashFolderAsync(tmpDir, { excludeFolders: ["dist"] });

        expect(hash1).toBe(hash2);
    });

    it("produces the same hash as sync standalone", async () => {
        writeFileSync(join(tmpDir, "file.txt"), "content");
        mkdirSync(join(tmpDir, "sub"), { recursive: true });
        writeFileSync(join(tmpDir, "sub", "nested.txt"), "nested");

        const syncHash = hashFolder(tmpDir);
        const asyncHash = await hashFolderAsync(tmpDir);
